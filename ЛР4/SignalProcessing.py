...
for M in M_list:
    ...
    quantize_levels = np.arange(
        np.min(quantized_signal),
        np.max(quantized_signal) + 1,
        delta
    )
    ...
    quantize_bit = [
        format(
            value,
            '0' + str(int(np.log(M) / np.log(2))) + 'b'
        )
        for value in quantize_bit
    ]
    ...
    table = ax.table(
        ...
        colLabels=['Signal value', 'Bit code'],
        loc='center'
    )
    ...
    fig.savefig(
        f'./figures/quantization_table_M_{M}.png',
        dpi=600,
        bbox_inches='tight'
    )
    ...
    ax.step(
        np.arange(0, len(bit_signal)),
        bit_signal,
        linewidth=0.5
    )

    ax.set_title(f'Bit sequence for M = {M}', fontsize=14)
    ax.set_xlabel('Bit index', fontsize=14)
    ax.set_ylabel('Bit value', fontsize=14)

    ax.grid(True)

    fig.savefig(
        f'./figures/bit_sequence_M_{M}.png',
        dpi=600
    )

    plt.close(fig)

    error = filtered_signal - quantized_signal

    variance_error = np.var(error)
    variance_signal = np.var(filtered_signal)

    quantization_variances.append(variance_error)

    snr = variance_signal / variance_error
    quantization_snr.append(snr)
...
fig.suptitle('Quantized digital signals', fontsize=14)
fig.supxlabel('Time (s)', fontsize=14)
fig.supylabel('Amplitude', fontsize=14)

fig.savefig(
    './figures/quantized_signals.png',
    dpi=600
)
...
plt.title(
    'Variance vs quantization levels',
    fontsize=14
)
...
plt.savefig(
    './figures/quantization_variance.png',
    dpi=600
)
